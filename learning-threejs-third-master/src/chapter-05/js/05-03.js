function init() {
  ...
  var controls = new function () {
    ...
    // innerRadius 圆环的内半径
    this.innerRadius = 3;
    // 圆环的外半径，它定义圆环的尺寸，这个半径是指从圆心到圆弧的距离。
    // 默认值为50 
    this.outerRadius = 10;
    // 用来定义创建圆环的对角线段的数量，较高的值意味着更平滑的圆环
    this.thetaSegments = 8;
    // 该属性定义沿着圆环的长度所需要使用的线段的数量。并不会真的影响圆的平滑度，但会增加面的数量
    this.phiSegments = 8;
    // 该属性定义从哪里开始画圆，这个值的范围是0-2PI 
    this.thetaStart = 0;
    // 该属性定义圆要画多大。如果没有指定，默认值为2*PI 例如这个值指定的是0.5PI 那么得到是四分之一圆
