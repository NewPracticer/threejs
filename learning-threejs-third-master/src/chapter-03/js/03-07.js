function init() {
  ...
  // size 可以用来指定光晕多大。这个尺寸的单位是像素，如果将它指定为-1 那么将使用纹理本身的尺寸
  // distance 从光源0到摄像机1的距离，使用这个参数可以将镜头光晕放置在正确的位置
  // blending(混合) 我们可以为光晕提供多种材质。混合模式决定了如何将它们混合在一起
  // opacity(不透明度) 定义光晕的透明度。0代表完全透明，1代表完全不透明
